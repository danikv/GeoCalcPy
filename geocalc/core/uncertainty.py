...
def J6_cartesian2spherical(x, y, z, vx, vy, vz):
    ...
    frac_helper_61 = - (vx*z) * (2*(x**2) * (r**2) - (y**2) * (R**2)) - (vy*x*y*z) * (2*(r**2) + (R**2)) - (vz*x) * (r**2) * ((z**2) - (r**2))
    # - (vx*z) * (2*(x**2) * (r**2) - (y**2) * (R**2)) = - (vx*z) * (2*(x**4) + (x**2)*(y**2) - y**4 - (y**2)*(z**2))
    # - (vy*x*y*z) * (2*(r**2) + (R**2)) = - (vy*x*y*z) * (3*(x**2) + 3*(y**2) + (z**2))
    # - (vz*x) * (r**2) * ((z**2) - (r**2)) = - (vz*x) * ((x**2)*(z**2) + (y**2)*(z**2) - (x**4) - 2*(x**2)*(y**2) - (y**4))
    frac_helper_63 = - (vy*z) * (2*(y**2) * (r**2) - (x**2) * (R**2)) - (vx*y*x*z) * (2*(r**2) + (R**2)) - (vz*y) * (r**2) * ((z**2) - (r**2))
    # we just chenged x <-> y, and vx <-> vy

    J6[6][1] = -frac_helper_61/((R**4)*(r**3))
    J6[6][2] = (x*z)/((R**2)*r)
    J6[6][3] = -frac_helper_63/((R**4)*(r**3))
    J6[6][4] = (y*z)/((R**2)*r)
    J6[6][5] = -((x*vx+y*vy)*(x**2+y**2-z**2)+2*z*vz*(x**2+y**2))/((R**4)*r)
    J6[6][6] = r/(R**2)
